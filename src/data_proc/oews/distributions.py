...
# %%
path_oews_national = '/project/high_tech_ind/high_tech_ind_job_flows/data/OEWS/national/proc/'

colums = ["OCC_CODE", "TOT_EMP"]

files = os.listdir(path_oews_national)

for file in files:
    print(f"Processing {file}...")
    year = re.search(r'\d{4}', file).group()
    df_oews = pd.read_csv(path_oews_national + file, usecols=colums)
    # Convert TOT_EMP to numeric
    df_oews['TOT_EMP'] = pd.to_numeric(df_oews['TOT_EMP'], errors='coerce')
    # Merge O*Net and OEWS data on OCC_CODE
    df_oews = pd.merge(df_oews, df_onet, on='OCC_CODE', how='left').dropna()
    # Convert TOT_EMP to to percentage of total employment
    df_oews.loc[:, "EMP"] = df_oews.TOT_EMP/df_oews.TOT_EMP.sum()
    # Add AREA = "00000" and AREA_NAME = "National" as columns
    df_oews.loc[:, "AREA"] = "00000"
    df_oews.loc[:, "AREA_NAME"] = "National"
    # Re-order columns
    df_oews = df_oews[["AREA", "AREA_NAME", "OCC_CODE", "EMP", "SKILL"]]
    # Save to csv
    df_oews.to_csv(f"/project/high_tech_ind/high_tech_ind_job_flows/data/OEWS/estimated_skill_distributions/national_skill/national_skill_{year}.csv", index=False)
