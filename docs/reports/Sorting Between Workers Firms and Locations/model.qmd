...
- Firms have access to a production technology that operates at the match level and depends on the location and the aggregate state of the economy, represented as $f(x, y, j)$.
  - Worker productivity is influenced by the distribution of skills within each location (see Chapter -@sec-agglomeration_effect for more details).
...
Firms will post vacancies such that the marginal cost of the vacancies and the marginal expected benefit $B^j$ are equal: $$c_{j}'(v^{j}(y))=q^{j}B^{j}(y)$$ {#eq-vacancy-posting-clearing}
...
<!-- -   And $\hat{\phi}^j_u(x,j')$ the probability that a type $x$ unemployed worker from location $j$ search in location $j'$ (this the optimal search policy in \eqref{eq-optimal-search-unemployed}. -->
