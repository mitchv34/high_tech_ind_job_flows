...
<!-- TODO: Fix this derivation to reflext the changes to the model -->
...
$$\phi(j') = \frac{e^{\left(\mathbb{E}_t\left[U^{j'} - F^{j \to j'}\right] /c_1\right)}}{\sum_{\tilde{j}\in \mathcal{J}}e^{\left(\mathbb{E}_t\left[U^{\tilde{j}} - F^{j \to \tilde{j}}\right] /c_1\right)}}$${#eq-optimal-search-unemp}
