...
-   Assume that a type $x$ worker in location $j$ has a $B(x, \overline{X}^j)$ where $\overline{X}^j$ is the value of idea exchange in location $j$. Thus: $$f(x,y,j,z_t) = f\left(B(x, \overline{X}^j), y, z_t\right)$$
