...
\begin{equation}
u^{j}_{t+1}(x) = \sum_{j'\in \mathcal{J}} \underbrace{\phi_u^{j'}(x,j)}u^{j'}_{t+}(x)\left( 1 - \int \eta^{j'\to j}(x,y) p^j\frac{v^j(y)}{V^j}dy \right)
\end{equation}

and of the distribution of employment

\begin{align}
h^{j}_{t+1}(x,y) &= \underbrace{h_{t+}(x,y)}_{\text{employed at interim }} \times \overbrace{\prod_{j'\in\mathcal{J}}\left[ \underbrace{1 - \frac{s}{\mid \mathcal{J} \mid}\int\left( p^{j'}_t\frac{v^{j'}_t(y')}{V^{j'}_t} \eta^{j\to j'}(x,y\to y')\right)dy'}_{\text{no poeached by any firm in location }j'} \right]}^{\text{probability of not being poached}}+\nonumber
\\
&\hspace{6cm} +\sum_{j'\in \mathcal{J}}\left(\overbrace{\int\underbrace{ h^{j'}_{t+}(x,y')sp^j_t\frac{v^{j}_t(y)}{V^{j}_t}\eta^{j'\to j}(x,y'\to y)}_{\text{mass of workers poached from } y' \text{ firms in location } j'}dy'}^{\text{mass of workers that } y \text{ succesfully poach from location} j'} \right) \nonumber \\
&\hspace{6cm} +  \underbrace{\sum_{j'\in \mathcal{J}}\left( u^{j'}_{t+}(x)\hat{\phi}_{u}^{j'}(x, j)p_t\frac{v^{j}_t(y)}{V^{j}_t}\eta^{j' \to j}(x,y) \right)}_{\text{mass of workers hired from unemployment}}
\end{align}
...
\hypertarget{edogenizing-location-productivity-and-cost-of-living}{%
\chapter{Edogenizing Location Productivity and Cost of
Living}\label{edogenizing-location-productivity-and-cost-of-living}}
...
\item
  ...
  \[f(x,y,j,z_t) = f\left(B(x, \overline{X}^j), y, z_t\right)\]
\item
...
f(x,y,j,z_t) = z_t B(x,\overline{X}^j)^\alpha y^{1-\alpha}
