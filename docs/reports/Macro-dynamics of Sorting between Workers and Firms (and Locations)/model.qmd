...
\end{align}
```
We can compute the distribution of skill in each location as: $$\ell^j_{t+1}(x) = u^j_{t+}(x) + \int h^j_{t+}(x,y)dy$$ and the total population in each location as: $$\mu^j_{t+1} = \int \ell^j_{t+}(x)dx$$