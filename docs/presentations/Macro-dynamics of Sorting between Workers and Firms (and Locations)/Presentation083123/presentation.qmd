...
-   And $\hat{\phi}^j_u(x,j')$ the probability that a type $x$ unemployed worker from location $j$ search in location $j'$.

## New Unemployment {.smaller}

- The law of motion of the unemployment rate is:

```{=tex}
\begin{equation}
u^{j}_{t+1}(x) = \sum_{j'\in \mathcal{J}} \phi_u^{j'}(x,j)u^{j'}_{t+}(x)\left( 1 - \int \eta^{j'\to j}(x,y) p^j\frac{v^j(y)}{V^j}dy \right)
\end{equation}
```
